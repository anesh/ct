...
c = conn.cursor()
c.execute('''CREATE TABLE vipobjs (poolname text,poolmember text,monitor text)''')



device=raw_input('Enter Device :')

outputFile = open('poolmembers.txt', 'w+')
...
for part in partlist:
  ...
  for pool in pools:
    ...
    try:
	  ...
	  c.execute('INSERT INTO vipobjs VALUES (?,?,?)',(rmpx,addcomb,statx[inc]))
	  ...
	  c.execute('INSERT INTO vipobjs VALUES (?,?,?)',(rmpool,z,monstat))
